...
{
    public class LogoffAction : RemoteAction
    {
        ManagementPath oRootPath;
        ManagementScope oRootMs;
        ConnectionOptions oCon = new ConnectionOptions();

         public LogoffAction(Dictionary<RemoteActionState, ImageSource> images)
            :base(images)
        {
            this.RemoteActionName = "Initiate Forced LogOff";
        }
            ...
            if (pc.Status == ComputerStates.Online || pc.Status == ComputerStates.LoggedOn)
            {
                ...
                try
                {
                    ...
                    foreach (ManagementObject mo in oSearcher.Get())
                    {
                        ...
                        inParams["Flags"] = 4; //ForcedLogoff; -- fixed to Logoff.
                        inParams["Reserved"] = 0;
